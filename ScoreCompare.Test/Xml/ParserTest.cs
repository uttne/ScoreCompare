...
namespace ScoreCompare.Test.Xml
{
    ...
    public class ParserTest
    {
        ...
        [TestMethod]
        public void Parse_success_of_xml_including_newline()
        {
            var target = new Parser();
...
<tag1>
    ...
    <tag3>data</tag3>
            ...
            Console.WriteLine($"{nameof(expected)} : {string.Join("|",expected.Select(x=>x.ToString()))}");
            Console.WriteLine($"{nameof(actual)} : {string.Join("|", actual.Select(x => x.ToString()))}");

            CollectionAssert.AreEqual(expected, actual);
        }

        [TestMethod]
        public void Parse_success_of_xml_including_xmlDexlaration()
        {
            var target = new Parser();
